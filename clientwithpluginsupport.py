...
# Define the send_message and constantly_active functions
def send_message(message):
   client.send(message.encode('ascii'))

def constantly_active():
   while True:
       pass
...
def load_plugin(file_path):
   ...
   if not hasattr(plugin, 'send_message'):
       plugin.send_message = send_message
   if not hasattr(plugin, 'constantly_active'):
       plugin.constantly_active = constantly_active
   return plugin
